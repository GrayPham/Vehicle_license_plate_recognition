...
namespace Vehicle_license_plate_recognition.DAL.OfficeStaff
{
    internal class SearchContractCDAO : QuanLiNhaXe
    {
        ...
        {
            var contact = db.Contracts.Where(u => u.isDelete == null).Select(u => new
            {
                ID = u.Id,
                NAME = u.NameContract,
        ...
        {
            var contact = db.Contracts.Where(u => u.ExpiryDate.Value < DateTime.Now && u.isDelete != null).Select(u => new
            {
                ID = u.Id,
                NAME = u.NameContract,
                CUSTOMER = u.IdRenter,
